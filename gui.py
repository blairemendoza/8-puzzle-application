...
class GUI:
  def __init__(self):
    ...
    for i in range(0,3):
      ...
      for j in range(0,3):
        ...
        if not (i == 0 and j == 0):
          self.labels[i][j] = Button(self.boardFrame, text=str(i*3+j), font=('Arial 30 bold'), height=4, width=10, bd='1', bg='#234675', fg='#e1e4e8', activebackground='#1a375e',activeforeground='#e1e4e8') 
    ...
    self.BottomFrame = Frame(self.mainFrame, bg="#273242")
    self.labelsFrame = Frame(self.BottomFrame, bg="#273242", padx='10px', pady='5px')
    self.ButtonsFrame = Frame(self.BottomFrame, bg="#161b24")
    Grid.rowconfigure(self.BottomFrame, 0, weight=1)
    Grid.columnconfigure(self.BottomFrame, 0, weight=1)
    
    self.loadingLabel = Label(self.labelsFrame, text='Shuffle the tiles. Click them to move.', bg="#273242", fg='#e1e4e8', font=('Arial 12 bold'))
    self.startButton = Button(self.ButtonsFrame, text=' Go > ', bd='0', bg='#161b24', fg='#e1e4e8', activebackground='#0b0d0f', activeforeground='#e1e4e8', font=('Arial 20 bold'), command=lambda:self.start())
    
    
    self.backButton = Button(self.ButtonsFrame, text=' < Back ', bd='0', bg='#161b24', fg='#e1e4e8', activebackground='#0b0d0f', activeforeground='#e1e4e8', font=('Arial 20 bold'), command =lambda:self.backButtonAction())
  ...
  def start(self):
    ...
    self.buttons = []
    for i in range(0,2):
      button = Button(self.ButtonsFrame, bd='0', bg='#161b24', fg='#e1e4e8', activebackground='#0b0d0f', activeforeground='#e1e4e8', font=('Arial 20 bold'))
    ...
    self.endButton = Button(self.ButtonsFrame, text=' End >>', bd='0', bg='#161b24', fg='#e1e4e8', activebackground='#0b0d0f', activeforeground='#e1e4e8', font=('Arial 20 bold'), command =lambda:self.endButtonAction())
    self.costLabel = Label(self.labelsFrame, bg="#273242", fg='#e1e4e8')
    self.exploredLabel = Label(self.labelsFrame, bg="#273242", fg='#e1e4e8')
    self.depthLabel = Label(self.labelsFrame, bg="#273242", fg='#e1e4e8')
    self.timeLabel = Label(self.labelsFrame, bg="#273242", fg='#e1e4e8')
    self.stepsLabel = Label(self.labelsFrame, bg="#273242", fg='#e1e4e8')
  ...
  def startHomePage(self):
    ...
    for i in range(0, len(self.buttons)):
      self.buttons[i].pack(side=LEFT,expand=YES,fill=BOTH)
    self.loadingLabel.config(text = "Choose an algorithm to solve the puzzle")
    self.loadingLabel.pack()
  ...
  def startShufflePage(self):
    ...
    self.currentStep = 0
    self.display()
    self.loadingLabel.config(text = "Shuffle the tiles. Click them to move.")
    self.startButton.pack(side=RIGHT)
  ...
  def solveButtonAction(self,solver):
    ...
    self.stepsLabel.config(text=f'Step: {self.currentStep}/{self.noOfSteps}')
